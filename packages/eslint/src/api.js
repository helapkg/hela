...
async function smartLintFiles({ include, exclude, ...options }) {
  ...
  const engine = new CLIEngine({ ...opts, ignore: true });
  const alwaysHook = createAlwaysHook(opts, engine, report);

  const useIterables = false;
